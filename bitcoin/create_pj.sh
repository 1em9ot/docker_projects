#!/bin/bash
# create_causal_quant_app.sh
# 因果推論＋説明可能性対応の Dash トレードアプリを
# Docker コンテナ bitflyerlightningapp として構築・起動するスクリプト
set -e

# --- 1. 前提チェック ---
# ポート 8050 が開いているか
if lsof -i :8050 >/dev/null; then
  echo "Error: Port 8050 is already in use. Please free it or modify the script."
  exit 1
fi
# .env ファイルがあるか
if [ ! -f .env ]; then
  echo "Error: .env file not found in current directory."
  echo "Place your BITFLYER_API_KEY, BITFLYER_API_SECRET, NEWSAPI_KEY etc. in .env"
  exit 1
fi
...
if [ -d "$PROJECT_DIR" ]; then
  echo "Found existing '$PROJECT_DIR'. Backing up persistent/ then recreating..."
...
def fetch_ohlcv(symbol='BTC', interval='1h', limit=500):
    ...
    r = pd.read_json(requests.get(url, params=params).text)
    # 省略せずに完全実装
...
def backtest_with_significance(df, short_w=20, long_w=50):
    ...
    buy_samples = df.loc[df['signal_change']==1, 'future_return'].dropna()
    if len(buy_samples) >= 2:
        _, pval = ttest_1samp(buy_samples, popmean=0, alternative='greater')
    else:
        pval = 1.0
...
def predict_next_prob(model, df):
    d = df.copy()
    d['SMA20'] = d['close'].rolling(20).mean()
    d['EMA20'] = d['close'].ewm(span=20).mean()
    from ta.momentum import rsi
    d['RSI14'] = rsi(d['close'], window=14)
    feat = d[['SMA20','EMA20','RSI14']].iloc[-1:].dropna()
    if feat.empty:
        return None
...
app.layout = dbc.Container(fluid=True, children=[
    ...
    dbc.Row([
        dbc.Col([html.Label("時間足:"), dcc.Dropdown(
            id='interval',
            options=[{'label':'1分','value':'1m'},{'label':'5分','value':'5m'},
                     {'label':'1時間','value':'1h'},{'label':'4時間','value':'4h'},
                     {'label':'1日','value':'1d'}],
        ...
        dbc.Col([html.Label("表示範囲:"), dcc.DatePickerRange(
            id='date-range',
            start_date=two_weeks_ago, end_date=today,
            min_date_allowed=today - timedelta(days=365),
            max_date_allowed=today, display_format='YYYY-MM-DD'
        )], width=3),
    ], className="my-3"),
...
def update_graphs(interval, short_w, long_w, limit, alpha, start_date, end_date, n):
    ...
    # 次足予測＋SHAP
    rf        = train_rf(df)
    p_up      = predict_next_prob(rf, df)
    explainer = shap.TreeExplainer(rf)
    shap_vals = explainer.shap_values(df.dropna()[['SMA20','EMA20','RSI14']])
    logging.info(f"SHAP mean: {np.mean(shap_vals, axis=1)}")

    # 日付フィルタ
    start = pd.to_datetime(start_date)
    end   = pd.to_datetime(end_date)
    df_bt  = df_bt[(df_bt.index>=start)&(df_bt.index<=end)]
    ...
    prob_txt = f"Next↑Prob: {p_up*100:.1f}%" if p_up else "Next↑Prob:N/A"
    fig1.add_annotation(
        text=f"α={alpha:.3f}, p-value={pval:.3f}, {prob_txt}",
...
echo "Setup complete. Access the app at http://localhost:8050"
echo "Project sources: $PROJECT_DIR/app/app.py"
echo "Logs will be in $PROJECT_DIR/persistent/app.log"
