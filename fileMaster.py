...
class WizTreeProcessor:
    ...
    async def process_csv(self, csv_file, log_callback):
        ...
        # プロセスプールは単一のものを使用（ワーク・スティーリングのため共有キューにより自動負荷分散）
        ...
        async for df_chunk in self.csv_reader.async_read_csv_blocks(csv_file, self.config.chunk_size, self.config.block_size):
            ...
            task = loop.run_in_executor(
                process_pool,
                self.db_handler.index_chunk_sync,
                df_chunk,
                self.config.base_thread_pool_workers,
                self.config
            )
            def _callback(fut, idx=chunk_idx, start=t_chunk_start):
                t = time.perf_counter() - start
                print(f"[Profile] Chunk {idx} processed in {t:.2f} sec")
        ...
        # 最終的な並列度は、ここではCPUコア数（プロセスプールのワーカー数）となる
