...
class DBHandler:
    ...
    def insert_records(conn, records):
        with conn.cursor() as cursor:
            ...
            sql = (
                f"INSERT INTO documents_csv "
                f"(file_id, file_path, file_name, size, alloc, updated, attributes, file_count, folder) "
                f"VALUES ({placeholders})"
            )
            cursor.executemany(sql, records)

# ---------------------------
# async_enumerate: 非同期版enumerate
# ---------------------------
...
class WizTreeProcessor:
    ...
    async def process_csv(self, csv_file, log_callback):
        ...
        tasks = []
        chunk_count = 0
        async for idx, df_chunk in async_enumerate(self.csv_reader.async_read_csv_blocks(csv_file, self.config.chunk_size, self.config.block_size)):
            logs.append(f"Received chunk {idx} with {len(df_chunk)} rows")
            tasks.append((self._timed_index_chunk_sync, (df_chunk, self.config.base_thread_pool_workers, self.config), {}))
            chunk_count += 1
        ...
        if chunk_timings:
            ...
            summary = (
                f"[Aggregate Profile] {total_chunks} chunks processed. Total processing time: {pipeline_elapsed:.2f} sec. "
...
class WizTreeCombinedTab:
    ...
    def create_widgets(self):
        lbl = tk.Label(self.frame, text="WizTree Processing\nSelect WizTree CSV File")
        lbl.pack(pady=5)
